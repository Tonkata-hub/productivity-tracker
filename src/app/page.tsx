...
import {
  CheckCircle2, Circle, Droplets, Activity, Scale,
  Utensils, Moon, FileText, Flame,
} from 'lucide-react'
...
export default function HomePage() {
  ...
  const toggleTask = async (task: TaskWithStatus) => {
    ...
    setTodayCompletions(prev => {
      ...
      wasCompleted ? n.delete(key) : n.add(key)
      return n
    })
    ...
    if (task.type === 'daily') {
      if (wasCompleted) {
        ...
        await supabase.from('task_completions').insert({ task_id: task.id, date: today, completed_at: new Date().toISOString() })
      }
    } else {
      await supabase
        .from('tasks')
        ...
        .eq('id', task.id)
      setTasks(prev =>
        prev.map(t => t.id === task.id ? { ...t, is_completed: !wasCompleted, completed_at: wasCompleted ? null : new Date().toISOString() } : t)
      )
    }
      ...
      <div className="max-w-xl mx-auto px-4 pt-6 pb-24 space-y-5">
        ...
        <div className="space-y-2 calendar-animate-slide-in-up" style={{ animationDelay: '160ms' }}>
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-muted-foreground">{completedCount} / {todayTasks.length}</span>
          </div>
          {todayTasks.length === 0 ? (
...
function StatCard({
  ...
  return (
    <div className={cn('glass rounded-2xl p-4 flex flex-col gap-0.5', accent && 'border-accent/20')}>
      ...
      <p className={cn('text-2xl font-bold tracking-tight', accent ? 'text-accent' : 'text-foreground')}>
        {value}
      </p>
...
function TaskRow({
  ...
  return (
    <div className={cn(
      ...
      {task.priority && (
        <div
          className={cn('w-1.5 h-1.5 rounded-full flex-shrink-0', priorityDot[task.priority] ?? '')}
          title={task.priority}
        />
      )}
    </div>
